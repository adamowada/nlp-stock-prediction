"""Persisted Evaluation Stage outcome-evaluation source loading."""
...
class OutcomeEvaluationSource:
    """A persisted outcome-evaluation artifact ready for Evaluation Stage cohort tools."""
...
def load_outcome_evaluation_sources(
    *,
    store: SQLiteStore,
    repo_root: Path,
    run_id: str,
) -> tuple[OutcomeEvaluationSource, ...]:
    """Load and validate persisted outcome-evaluation payloads for Evaluation Stage tooling."""
...
def _source_from_record(
    *,
    ...
    try:
        ...
        raise ValueError(
            "Persisted outcome-evaluation artifact is not a valid Evaluation Stage payload: "
            f"{artifact.artifact_id}"
        ) from exc
    _validate_source_payload(run_id=run_id, record=record, artifact=artifact, payload=payload)
    return OutcomeEvaluationSource(
        record=record,
        artifact=artifact,
