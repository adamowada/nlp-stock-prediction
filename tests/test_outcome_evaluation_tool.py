...
def _store(tmp_path: Path) -> SQLiteStore:
    ...
    store.upsert_research_run(
        ResearchRunRecord(
            run_id=RUN_ID,
            run_kind="evaluation_outcome_evaluation",
    ...
    store.record_artifact(
        ArtifactRecord(
            ...
            path=Path("reports/run-evaluation-outcome/audit/news.json"),
            sha256="a" * 64,
    ...
    store.record_artifact(
        ArtifactRecord(
            ...
            artifact_type="normalized_evidence",
            path=Path("reports/run-evaluation-outcome/audit/news-after-cutoff.json"),
            sha256="f" * 64,
            schema_version="normalized-evidence.v1",
    ...
    store.record_artifact(
        ArtifactRecord(
            ...
            path=Path("reports/run-evaluation-outcome/audit/technicals.json"),
            sha256="b" * 64,
    ...
    store.record_artifact(
        ArtifactRecord(
            ...
            artifact_type="technical_package",
            path=Path("reports/run-evaluation-outcome/audit/technicals-lookahead.json"),
            sha256="c" * 64,
            schema_version="technical-package.v1",
            produced_by="research_technical_package",
            record_count=12,
    ...
    store.record_artifact(
        ArtifactRecord(
            ...
            artifact_type="technical_package",
            path=Path("reports/run-evaluation-outcome/audit/technicals-future-asof.json"),
            sha256="c" * 64,
            schema_version="technical-package.v1",
            produced_by="research_technical_package",
            record_count=12,
    ...
    store.record_artifact(
        ArtifactRecord(
            ...
            path=Path("reports/run-evaluation-outcome/audit/prediction-input.json"),
            sha256="d" * 64,
    ...
    store.record_artifact(
        ArtifactRecord(
            ...
            path=Path("reports/run-evaluation-outcome/audit/market-observation.json"),
            sha256="e" * 64,
...
def test_build_prediction_evaluation_target_reviews_current_artifact_file_state(
    ...
    store = _store(tmp_path)
    artifact_path = tmp_path / "reports" / "run-evaluation-outcome" / "audit" / "technicals.json"
    ...
    reliability_freshness = cast(dict[str, object], target.metadata["reliability_freshness"])
    reviews = cast(list[dict[str, object]], reliability_freshness["artifact_freshness_reviews"])
    cutoff_review = next(
...
def test_observed_outcome_rejects_unavailable_or_unattributed_sources(tmp_path: Path) -> None:
    ...
    store.record_artifact(
        ArtifactRecord(
            ...
            artifact_type="market_data",
            path=Path("reports/run-evaluation-outcome/audit/market-other.json"),
            sha256="f" * 64,
            schema_version="market-data.v1",
            created_at=OBSERVED_AT,
