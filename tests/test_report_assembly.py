...
def test_signal_artifact_policy_maps_run_artifacts_to_typed_families() -> None:
    records = (
        ArtifactRecord(
            ...
            sha256="sha-market",
            schema_version="research-market-data.v1",
            produced_by="research_market_data",
            created_at=NOW,
        ),
        ArtifactRecord(
            ...
            sha256="sha-social",
            schema_version="research-social.v1",
            produced_by="research_social_evidence",
        ...
        ArtifactRecord(
            ...
            sha256="sha-sector",
            schema_version="research-sector-macro.v1",
            produced_by="research_sector_macro",
            created_at=NOW,
        ),
    )
...
def test_report_assembly_traces_candidates_to_stored_evidence_and_artifacts(
    ...
    result = service.run_offline_research_flow(
        ...
        output_dir="reports/report-report-assembly",
        symbol="TSLA",
    )
...
def test_report_assembly_uses_explicit_candidate_link_tables(
    ...
    service.store.record_tool_run(
        ToolRunRecord(
            ...
            run_id=run_id,
            tool_name="report_link_table_test",
    ...
    service.store.record_artifact(
        ArtifactRecord(
            ...
            tool_run_id="tool-linked-prediction-input",
            produced_by="report_link_table_test",
...
def test_report_assembly_rerender_does_not_reingest_final_report_artifacts(
    ...
    result = service.run_offline_research_flow(
        run_date=RUN_DATE.isoformat(),
        output_dir="reports/report-report-rerender",
        symbol="TSLA",
    )
...
def test_report_assembly_malformed_required_artifact_becomes_insufficient_evidence(
    ...
    service.store.record_artifact(
        ArtifactRecord(
            ...
            schema_version="research.technical-package.v1",
            tool_run_id="tool-bad-technical",
            produced_by="research_technical_package",
            record_count=1,
            metadata={"symbol": "TSLA"},
            created_at=NOW,
        )
...
def test_report_assembly_failed_provider_run_is_visible_in_report_and_audit_manifest(
    ...
    service.store.record_tool_run(
        ToolRunRecord(
            ...
            run_id=run_id,
            tool_name="research_news_catalyst",
            tool_version="research.news.v1",
            status="failed",
            started_at=NOW,
            completed_at=NOW,
            inputs={"symbol": "TSLA"},
...
def test_report_assembly_surfaces_ambiguous_and_unsupported_instrument_resolutions(
    ...
    service.store.record_artifact(
        ArtifactRecord(
            ...
            path=artifact_path.relative_to(tmp_path),
            sha256=hashlib.sha256(artifact_path.read_bytes()).hexdigest(),
            schema_version="research.instrument-universe.v1",
            tool_run_id="tool-stage8-universe",
            produced_by="research_universe_discovery",
            record_count=2,
            metadata={"symbol": "TSLA"},
            created_at=NOW,
        )
...
def _service_with_started_run(tmp_path: Path) -> tuple[ResearchService, str]:
...
def _candidate(*, signal_artifacts: tuple[str, ...]) -> PredictionCandidateRecord:
    return PredictionCandidateRecord(
        ...
        run_id=f"research-{RUN_DATE.isoformat()}-tsla",
        instrument_id="instrument:equity:us:tsla",
        prediction_horizon="swing",
        prediction_type="directional",
    ...
    *,
    ...
    service.store.record_tool_run(
        ToolRunRecord(
            tool_run_id="tool-bad-technical",
            run_id=run_id,
            tool_name="research_technical_package",
...
def _write_universe_resolution_artifact(
    *,
    ...
    payload = research_universe_artifact_payload(run_id=run_id, universe=universe)
