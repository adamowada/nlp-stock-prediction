"""Report rendering for Codex Smoke Stage Codex smoke runs."""
...
class PredictionReportBuildRequest:
    """Public request for building and indexing a final prediction report bundle."""
    ...
    artifact_schema_version: str = "codex_smoke-report.v1"
...
class ReportBundleBuilder:
    ...
    def build(self, request: PredictionReportBuildRequest) -> JsonObject:
        return _render_prediction_report_core(
            store=request.store,
...
def _render_prediction_report_core(
    *,
    store: SQLiteStore,
    repo_root: Path,
    run: ResearchRunRecord,
    ...
    is_research_report = artifact_schema_version.startswith("research")
    resolved_report_data_mode = report_data_mode or report_data_mode_from_run(
        run,
        default=(
            OFFLINE_FIXTURE_REPORT_DATA_MODE if is_research_report else CODEX_SMOKE_REPORT_DATA_MODE
        ),
    )
    enforce_live_report_input_boundary(
        store=store,
    ...
    if resolved_report_data_mode == LIVE_REPORT_DATA_MODE:
        ...
        observed_discussion_summary = (
            "Research Stage fixture-backed research tools imported evidence and context."
        )
        analysis_summary = (
            "Research Stage report rendering consumes stored tool evidence, analysis artifacts, "
            "prediction candidates, and prediction-quality evaluations."
        )
        universe_summary = f"Research Stage fixture-backed research universe for {symbol.upper()}"
        freshness_summary = (
            "Research Stage used deterministic fixture-backed tool outputs."
            if evidence_sources
            else "Research Stage has no imported evidence for this run."
        )
    else:
        observed_discussion_summary = (
        ...
        analysis_summary = (
            "Codex Smoke Stage validates orchestration and provenance; dummy tools keep the "
            "prediction conservative."
        )
        universe_summary = f"Codex Smoke Stage Codex smoke universe for {symbol.upper()}"
        freshness_summary = (
...
def _primary_instrument(
    ...
    *,
    ...
    return codex_smoke_instrument(symbol.upper(), fallback_generated_at)
...
def _live_unknown_instrument(symbol: str, retrieved_at: datetime) -> Instrument:
    ...
    return Instrument(
        ...
        provider_ids=(
            ProviderInstrumentId(
                provider=RESEARCH_LIVE_SYMBOL_PROVIDER,
