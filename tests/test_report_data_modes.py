...
from nlp_stock_prediction.orchestration.research_fixture_providers import _StaticJsonTransport
...
@pytest.mark.unit
def test_research_run_kind_does_not_infer_fixture_mode_without_metadata() -> None:
    run = ResearchRunRecord(
        run_id="research-2026-05-13-tsla",
        run_kind="research_tool_suite",
...
def test_research_full_flow_does_not_implicitly_resume_existing_run(tmp_path: Path) -> None:
...
@pytest.mark.integration
def test_offline_research_report_inputs_are_machine_marked(tmp_path: Path) -> None:
...
@pytest.mark.integration
def test_live_research_report_without_inputs_renders_insufficient_evidence(
    tmp_path: Path,
) -> None:
    service = ResearchService(repo_root=tmp_path)
    started = service.start_research_run(
...
@pytest.mark.integration
def test_live_research_report_rejects_fixture_or_dummy_leakage(tmp_path: Path) -> None:
    service = ResearchService(repo_root=tmp_path)
    started = service.start_research_run(
        run_date=RUN_DATE.isoformat(),
    ...
    service.store.record_tool_run(
        ToolRunRecord(
            ...
            run_id=run_id,
            tool_name="research_universe_discovery",
            tool_version="research.fixture.v1",
            status="successful",
            started_at=NOW,
            completed_at=NOW,
...
@pytest.mark.integration
def test_live_research_flow_uses_live_providers_and_degrades_to_insufficient_evidence(
    tmp_path: Path,
) -> None:
    service = ResearchService(
        repo_root=tmp_path,
    ...
    assert "tool:research_news_catalyst" in payload["insufficient_evidence"]["provider_names"]
    assert any(
        health["provider_name"] == "tool:research_news_catalyst" and health["status"] == "failed"
        for health in payload["provider_health"]
    )
    assert any(
        run.tool_name == "research_prediction_candidate_synthesis" and run.status == "empty"
        for run in service.store.list_tool_runs_for_run(run_id)
    )


@pytest.mark.integration
def test_live_research_flow_prefers_live_instrument_identity_when_fixture_records_exist(
    tmp_path: Path,
) -> None:
    service = ResearchService(
        repo_root=tmp_path,
...
def test_live_boundary_scans_candidate_instrument_provider_records(tmp_path: Path) -> None:
    ...
    service.store.upsert_instrument(
        InstrumentRecord(
            ...
            provider_ids=(
                {
                    "provider": "research-fixture-universe",
    ...
    assert any(
        ...
        and violation.value == "research-fixture-universe"
...
class _UnitLiveProviderFactory(ResearchLiveProviderFactoryProtocol):
