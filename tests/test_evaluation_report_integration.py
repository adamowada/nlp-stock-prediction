...
from nlp_stock_prediction.orchestration.report_builder import _primary_instrument
...
def _started_service(tmp_path: Path) -> tuple[ResearchService, str, Path]:
    service = ResearchService(repo_root=tmp_path)
    started = service.start_research_run(
        run_date=RUN_DATE.isoformat(),
        output_dir="reports/evaluation-report-integration",
        symbol="MSFT",
        objective="Evaluation Stage report integration test.",
    )
...
def _seed_candidate(service: ResearchService, run_id: str) -> None:
    ...
    service.store.record_tool_run(
        ToolRunRecord(
            tool_run_id="tool-evaluation-report-evidence",
            run_id=run_id,
    ...
    service.store.upsert_prediction_candidate(
        PredictionCandidateRecord(
            candidate_id="candidate-msft-evaluation-report",
            run_id=run_id,
            instrument_id=INSTRUMENT_ID,
...
def test_rendered_report_surfaces_evaluation_outcomes_and_calibration_artifacts(
    tmp_path: Path,
) -> None:
    ...
    prior_calibration = EvaluationService(repo_root=tmp_path).calibration_summary(
        run_id=run_id,
        cohort_id="evaluation-evalcal-stage8-msft-swing",
        as_of=(EVALUATED_AT + timedelta(days=1)).isoformat(),
        artifact_dir=audit_dir.as_posix(),
        bin_edges=(0.0, 0.5, 1.0),
    )
    calibration = EvaluationService(repo_root=tmp_path).calibration_summary(
        run_id=run_id,
        cohort_id="evaluation-evalcal-stage8-msft-swing",
    ...
    drift = EvaluationService(repo_root=tmp_path).calibration_drift_check(
        run_id=run_id,
    ...
    assert review["candidate_id"] == "candidate-msft-evaluation-report"
    ...
    trace = build_report_trace(
        ...
        audit_artifacts=(
            AuditArtifact(
                ...
                created_at=NOW,
                produced_by="calibration_summary",
                sha256="a" * 64,
