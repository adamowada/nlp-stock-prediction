"""First-class Research Stage instrument universe discovery tool."""
...
RESEARCH_UNIVERSE_SCHEMA_VERSION = "research.instrument-universe.v1"
RESEARCH_FIXTURE_PROVIDER = "research-fixture-directory"
RESEARCH_LIVE_SYMBOL_PROVIDER = "live-symbol-directory"
RESEARCH_TOOL_NAME = "research_universe_discovery"
RESEARCH_TOOL_VERSION = "research.fixture.v1"
RESEARCH_LIVE_TOOL_VERSION = "research.live.v1"
...
@dataclass(frozen=True)
class ResearchUniverseDiscoveryToolResult:
    """Persisted Research Stage universe discovery output."""
...
@dataclass(frozen=True)
class ResearchUniverseDiscoveryTool:
    """Resolve and persist an instrument universe through the Instrument-Universe Stage registry."""

    provider: UniverseDiscoveryProvider = field(
        default_factory=lambda: ResearchFixtureUniverseProvider()
    )
        ...
        *,
        ...
        with safe_research_tool_execution(
            ...
            artifact = ArtifactIndex.for_directory(
                ...
                produced_by=RESEARCH_TOOL_NAME,
                tool_run_id=tool_run_id,
                ...
                payload=research_universe_artifact_payload(
                    run_id=context.run_id,
                    universe=universe,
                ),
            ...
            return ResearchUniverseDiscoveryToolResult(
                universe=universe,
                artifact=artifact,
...
@dataclass(frozen=True)
class ResearchFixtureUniverseProvider:
    """Deterministic offline instrument discovery provider for Research Stage tests."""

    provider_name: str = RESEARCH_FIXTURE_PROVIDER
    provider_version: str = RESEARCH_TOOL_VERSION
...
@dataclass(frozen=True)
class ResearchLiveSymbolUniverseProvider:
    """Create live-mode instrument identities from explicit user symbol requests."""

    provider_name: str = RESEARCH_LIVE_SYMBOL_PROVIDER
    provider_version: str = RESEARCH_LIVE_TOOL_VERSION
...
def research_universe_artifact_payload(*, run_id: str, universe: InstrumentUniverse) -> JsonObject:
    """Serialize a Research Stage universe artifact with a stable top-level envelope."""
...
def instrument_record_from_contract(
    ...
    *,
    ...
    metadata: JsonObject = {
        "research_universe_discovery": True,
    ...
    metadata.update(
        {
            ...
            "source": "fixture" if provider_name == RESEARCH_FIXTURE_PROVIDER else "provider",
...
def _fixture_instrument(
    *,
    ...
    return Instrument(
        ...
        provider_ids=(
            ProviderInstrumentId(
                provider=RESEARCH_FIXTURE_PROVIDER,
                identifier=provider_identifier.upper(),
                namespace="fixture-symbol",
                url=source_url,
                metadata={"source": "research_fixture"},
            ),
        ),
        tradability_evidence=(
            TradabilityEvidence(
                ...
                raw_identifier=f"{provider_identifier.upper()}:research-fixture",
                notes=notes,
                metadata={"source": "research_fixture"},
            ),
        ),
        ...
        metadata={"stage": "research", "fixture": True},
    )
...
def _tool_run_id(run_id: str, request_id: str) -> str:
    return f"tool-research-universe-{_stable_digest(f'{run_id}:{request_id}')}"


def _artifact_id(run_id: str, request_id: str) -> str:
    return f"artifact-research-universe-{_stable_digest(f'{run_id}:{request_id}')}"
...
def _source_query_id(
    ...
    return f"query-research-universe-{digest}"


def _artifact_filename(request_id: str, run_id: str) -> str:
    return f"instrument-universe-{_stable_digest(f'{run_id}:{request_id}')}.json"


def _fixture_query_url(query: str) -> str:
    return f"https://example.com/research-fixtures/universe?q={quote(query, safe='')}"


def _fixture_instrument_url(provider_identifier: str) -> str:
    return (
        "https://example.com/research-fixtures/instruments/"
    ...
    return Instrument(
        ...
        provider_ids=(
            ProviderInstrumentId(
                ...
                metadata={
                    "provider_version": RESEARCH_LIVE_TOOL_VERSION,
                    "resolution_basis": "explicit_live_symbol_request",
                },
            ),
        ),
...
__all__ = [
    "RESEARCH_FIXTURE_PROVIDER",
    "RESEARCH_LIVE_SYMBOL_PROVIDER",
    "RESEARCH_LIVE_TOOL_VERSION",
    "RESEARCH_TOOL_NAME",
    "RESEARCH_TOOL_VERSION",
    "RESEARCH_UNIVERSE_SCHEMA_VERSION",
    "ResearchFixtureUniverseProvider",
    ...
    "UniverseDiscoverySourceQuery",
    "instrument_record_from_contract",
    "research_universe_artifact_payload",
]
