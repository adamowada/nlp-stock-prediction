...
def _seed_run(store: SQLiteStore, context: RunContext) -> None:
    ...
    store.upsert_research_run(
        ResearchRunRecord(
            ...
            run_kind="research_universe_discovery_test",
            objective="Fixture-backed Research Stage universe discovery test",
            status="running",
...
def test_research_fixture_discovery_persists_run_graph_and_indexed_instruments(
    tmp_path: Path,
) -> None:
    repo_root = tmp_path
    ...
    request = InstrumentUniverseRequest(
        request_id="research-fixture-universe",
        as_of=_now(),
        queries=(
    ...
    result = ResearchUniverseDiscoveryTool().run(
        request=request,
        context=context,
        store=store,
        repo_root=repo_root,
    ...
    assert tool_run.tool_name == RESEARCH_TOOL_NAME
    ...
    assert artifact.schema_version == RESEARCH_UNIVERSE_SCHEMA_VERSION
    ...
    assert artifact_payload["schema_version"] == RESEARCH_UNIVERSE_SCHEMA_VERSION
    ...
    assert ai_source_query.provider == RESEARCH_FIXTURE_PROVIDER
    ...
    assert tsla.metadata["research_universe_discovery"] is True
    assert tuple(
...
def test_research_fixture_discovery_keeps_ambiguous_unavailable_and_unsupported_explicit(
    tmp_path: Path,
) -> None:
    repo_root = tmp_path
    context = _context(repo_root)
    ...
    _seed_run(store, context)
    request = InstrumentUniverseRequest(
        request_id="research-resolution-outcomes",
        as_of=_now(),
        queries=(
            InstrumentQuery(query="AI"),
            InstrumentQuery(query="MISSING"),
...
def test_research_fixture_discovery_resolves_provider_hinted_fixture_identifier(
    tmp_path: Path,
) -> None:
    repo_root = tmp_path
    context = _context(repo_root)
    ...
    _seed_run(store, context)
    request = InstrumentUniverseRequest(
        request_id="research-provider-hinted-fixture",
        as_of=_now(),
        queries=(
            InstrumentQuery(
                query="Tesla Inc",
...
def test_research_fixture_discovery_requested_id_does_not_override_unrelated_query(
    tmp_path: Path,
) -> None:
    repo_root = tmp_path
    context = _context(repo_root)
    ...
    _seed_run(store, context)
    request = InstrumentUniverseRequest(
        request_id="research-requested-id-mismatch",
        as_of=_now(),
        watchlists=(
            Watchlist(
                watchlist_id="research-watchlist",
                name="Research Stage Watchlist",
                entries=(
                    WatchlistEntry(
