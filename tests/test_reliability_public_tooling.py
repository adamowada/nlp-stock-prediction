...
RUN_ID = "run-reliability-public-evaluation"
...
def _persist_outcome_evaluation(tmp_path: Path) -> EvaluationService:
    store = _store(tmp_path)
    _persist_candidate(store)
    service = EvaluationService(repo_root=tmp_path)
    service.point_in_time_outcome_evaluation(
        run_id=RUN_ID,
        candidate_id="candidate-public-msft",
...
def _persist_live_evidence(store: SQLiteStore) -> None:
    store.record_tool_run(
        ToolRunRecord(
            ...
            run_id=RUN_ID,
            tool_name="research_market_data",
...
@pytest.mark.unit
def test_reliability_public_service_writes_real_evaluation_artifacts(tmp_path: Path) -> None:
...
@pytest.mark.unit
def test_reliability_public_writer_tools_require_explicit_artifact_dir(tmp_path: Path) -> None:
    service = _persist_outcome_evaluation(tmp_path)
...
@pytest.mark.unit
def test_reliability_public_writer_tool_records_failure_after_artifact_write_error(
    tmp_path: Path,
) -> None:
    service = _persist_outcome_evaluation(tmp_path)
    audit_dir = tmp_path / "reports" / RUN_ID / "audit"
...
@pytest.mark.unit
def test_reliability_public_service_writes_source_reliability_notes(tmp_path: Path) -> None:
    store = _store(tmp_path)
...
@pytest.mark.unit
def test_reliability_registry_and_mcp_expose_public_evaluation_tools(tmp_path: Path) -> None:
    ...
    register_evaluation_mcp_tools(server, EvaluationService(repo_root=tmp_path))
