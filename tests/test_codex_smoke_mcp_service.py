...
from nlp_stock_prediction.orchestration import CodexSmokeMcpService


@pytest.mark.integration
def test_codex_smoke_mcp_service_records_search_evidence_and_renders_report(tmp_path: Path) -> None:
    service = CodexSmokeMcpService(repo_root=tmp_path)

    started = service.start_research_run(
        run_date="2026-05-13",
        output_dir="reports/codex_smoke-codex-smoke",
        symbol="TSLA",
    ...
    assert str(universe["universe_id"]).startswith("instrument_universe-fixture-universe-")
...
@pytest.mark.unit
def test_codex_smoke_mcp_service_restricts_write_roots(tmp_path: Path) -> None:
    service = CodexSmokeMcpService(repo_root=tmp_path)
...
@pytest.mark.unit
def test_codex_smoke_mcp_service_rejects_symbol_mismatch(tmp_path: Path) -> None:
    service = CodexSmokeMcpService(repo_root=tmp_path)
    started = service.start_research_run(
        run_date="2026-05-13",
        output_dir="reports/codex_smoke-codex-smoke",
...
@pytest.mark.unit
def test_codex_smoke_mcp_service_rejects_duplicate_deterministic_run_id(tmp_path: Path) -> None:
    service = CodexSmokeMcpService(repo_root=tmp_path)
    service.start_research_run(
        run_date="2026-05-13",
        output_dir="reports/codex_smoke-codex-smoke",
...
@pytest.mark.integration
def test_codex_smoke_mcp_service_synthesizes_contradictory_evidence(tmp_path: Path) -> None:
    service = CodexSmokeMcpService(repo_root=tmp_path)
    started = service.start_research_run(
        run_date="2026-05-13",
        output_dir="reports/codex_smoke-codex-smoke",
...
@pytest.mark.integration
def test_codex_smoke_mcp_service_neutral_only_evidence_stays_insufficient(tmp_path: Path) -> None:
    service = CodexSmokeMcpService(repo_root=tmp_path)
    started = service.start_research_run(
        run_date="2026-05-13",
        output_dir="reports/codex_smoke-codex-smoke",
