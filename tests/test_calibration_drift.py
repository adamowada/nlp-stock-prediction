...
RUN_ID = "run-reliability-calibration-drift"
COHORT_ID = "reliability-drift-msft-swing"
...
def test_evaluation_service_writes_calibration_drift_check(tmp_path: Path) -> None:
    store = initialize_database(tmp_path / "data" / "prediction-research.sqlite3")
    _seed_run(store)
    prior = _persist_calibration_source(
        store=store,
    ...
    result = service.calibration_drift_check(
        run_id=RUN_ID,
        prior_calibration_id=prior.calibration_id,
        current_calibration_id=current.calibration_id,
...
def _seed_run(store: SQLiteStore) -> None:
    store.upsert_research_run(
        ResearchRunRecord(
            run_id=RUN_ID,
            run_kind="reliability_calibration_drift",
    ...
    *,
    ...
    store.record_tool_run(
        ToolRunRecord(
            ...
            run_id=RUN_ID,
            tool_name="calibration_summary",
            tool_version="evaluation.calibration-summary.v1",
            status="successful",
    ...
    payload = CalibrationSummaryArtifactPayload(
        ...
        bin_edges=BIN_EDGES,
        summary=summary,
        metadata={"source": "reliability_drift_test"},
    )
    ...
    record = CalibrationRunRecord(
        calibration_id=summary.calibration_id,
        run_id=RUN_ID,
        method_version="evaluation.calibration-summary.v1",
        created_at=created_at,
