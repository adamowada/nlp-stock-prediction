"""FastMCP registration adapter for Codex Smoke Stage tools."""
...
from nlp_stock_prediction.orchestration.codex_smoke_tool_plan import CODEX_SMOKE_MCP_TOOL_NAMES


def register_codex_smoke_mcp_tools(server: Any, service: CodexSmokeMcpService) -> None:
    """Register Codex Smoke Stage tools on a FastMCP-compatible server."""
    ...
    def start_research_run(
        ...
        """Start a Codex Smoke Stage smoke research run and return run paths."""
    ...
    def list_research_tool_plan() -> dict[str, object]:
        """List the staged dummy tools Codex should call for Codex Smoke Stage smoke."""
    ...
    def run_dummy_universe_tool(run_id: str, symbol: str) -> dict[str, object]:
        """Write deterministic Instrument-Universe Stage fixture instrument-universe artifacts."""
    ...
    for tool_name in CODEX_SMOKE_MCP_TOOL_NAMES:
        server.tool()(functions[tool_name])


__all__ = ["register_codex_smoke_mcp_tools"]
