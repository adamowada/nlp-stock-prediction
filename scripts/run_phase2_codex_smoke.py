...
ALLOWED_SMOKE_WRITE_ROOTS = ("reports", "artifacts", "data", "cache")
...
class CodexSmokeConfig:
    ...
    def database_arg_path(self) -> Path:
        symbol_slug = _symbol_slug(self.symbol)
        filename = f"phase2-codex-smoke-{self.run_date.isoformat()}-{symbol_slug}.sqlite3"
...
def _expected_run_id(config: CodexSmokeConfig) -> str:
    return f"codex-smoke-{config.run_date.isoformat()}-{_symbol_slug(config.symbol)}"
...
def _is_allowed_smoke_path(path: Path, allowed_roots: tuple[Path, ...]) -> bool:
    return any(_is_relative_to(path, root) or path == root for root in allowed_roots)
