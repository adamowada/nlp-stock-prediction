"""Research Stage technical-package tool built from market-data artifacts."""
...
RESEARCH_TECHNICAL_PACKAGE_SCHEMA_VERSION = "research.technical-package.v1"
RESEARCH_TECHNICAL_PACKAGE_TOOL_NAME = "research_technical_package"
RESEARCH_TECHNICAL_PACKAGE_TOOL_VERSION = RESEARCH_TECHNICAL_PACKAGE_SCHEMA_VERSION
...
MarketDataInput = (
    MarketDataToolResult | ResearchMarketDataArtifact | ProviderResult[MarketSnapshot] | Path | str
)
...
class ResearchTechnicalPackageArtifact(ContractModel):
    """Stable JSON payload written by the Research Stage technical-package tool."""

    schema_version: NonEmptyStr = RESEARCH_TECHNICAL_PACKAGE_SCHEMA_VERSION
    run_id: NonEmptyStr
...
class TechnicalPackageToolResult:
    ...
    artifact_payload: ResearchTechnicalPackageArtifact
    tool_run_id: str
...
class _MarketDataBundle:
    ...
    market_artifact: ResearchMarketDataArtifact | None
...
@dataclass(frozen=True)
class ResearchTechnicalPackageTool:
    ...
    def run(
        ...
        *,
        ...
        with safe_research_tool_execution(
            ...
            tool_version=RESEARCH_TECHNICAL_PACKAGE_TOOL_VERSION,
            ...
            artifact = ArtifactIndex.for_directory(
                ...
                produced_by=RESEARCH_TECHNICAL_PACKAGE_TOOL_NAME,
                tool_run_id=tool_run_id,
                schema_version=RESEARCH_TECHNICAL_PACKAGE_SCHEMA_VERSION,
...
def technical_package_artifact_payload(artifact: ResearchTechnicalPackageArtifact) -> JsonObject:
    """Serialize a Research Stage technical-package artifact to stable JSON-compatible data."""

    return cast(JsonObject, artifact.model_dump(mode="json"))


def load_research_technical_package_artifact(path: Path) -> ResearchTechnicalPackageArtifact:
    """Load a previously written technical-package artifact."""

    return ResearchTechnicalPackageArtifact.model_validate_json(path.read_text(encoding="utf-8"))
...
def _coerce_market_data_input(
    ...
    *,
    ...
    if isinstance(market_data, ResearchMarketDataArtifact):
        return _MarketDataBundle(
            provider_result=market_data.provider_result,
    ...
    try:
        artifact = load_research_market_data_artifact(path)
        return _MarketDataBundle(
...
def _warning(
    *,
    ...
    return provider_warning(
        provider_name=RESEARCH_TECHNICAL_PACKAGE_TOOL_NAME,
        code=code,
        severity=severity,
        message=message,
...
def _technical_package_tool_run_id(run_id: str, symbol: str) -> str:
    return f"tool-research-technical-package-{stable_digest(f'{run_id}:{symbol}')}"
...
__all__ = [
    ...
    "RESEARCH_TECHNICAL_PACKAGE_TOOL_VERSION",
    "TECHNICAL_PACKAGE_PREDICTION_POLICY",
    "MarketDataInput",
