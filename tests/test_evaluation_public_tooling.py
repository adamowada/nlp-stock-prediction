...
RUN_ID = "run-evaluation-public-tools"
...
def _store(tmp_path: Path) -> SQLiteStore:
    ...
    store.upsert_research_run(
        ResearchRunRecord(
            run_id=RUN_ID,
            run_kind="evaluation_and_calibration",
            objective="Run Evaluation Stage live tooling over persisted outcome evaluations.",
            status="running",
            started_at=NOW,
    ...
    *,
    ...
    signal_artifacts = tuple(
        SignalArtifactReference(
            ...
            produced_by=f"research_{family.value}",
            created_at=CUTOFF,
            as_of=CUTOFF,
            sha256="a" * 64,
...
@pytest.mark.unit
def test_evaluation_registry_exposes_live_evaluation_tool_suite() -> None:
    plan = build_evaluation_tool_registry().as_plan()
...
@pytest.mark.unit
def test_orchestration_facade_lazily_exports_evaluation_service() -> None:
    from nlp_stock_prediction.orchestration import (
        EvaluationService as ExportedEvaluationService,
    )
    from nlp_stock_prediction.orchestration import (
        build_evaluation_tool_registry as exported_registry_factory,
    )

    assert ExportedEvaluationService is EvaluationService
    assert exported_registry_factory().as_plan()["tools"]


@pytest.mark.unit
def test_evaluation_mcp_registration_matches_public_tool_names(tmp_path: Path) -> None:
    from nlp_stock_prediction.orchestration.evaluation_mcp_registration import (
        EVALUATION_MCP_TOOL_NAMES,
        register_evaluation_mcp_tools,
    )

    server = _FakeMcpServer()
    service = EvaluationService(repo_root=tmp_path)

    register_evaluation_mcp_tools(server, service)

    assert list(EVALUATION_MCP_TOOL_NAMES) == server.registered
    assert "run_dummy_universe_tool" not in server.registered
    assert "run_dummy_analysis_tool" not in server.registered


@pytest.mark.unit
def test_codex_mcp_server_registers_research_and_evaluation_tooling(
    tmp_path: Path,
    ...
    from nlp_stock_prediction.orchestration.research_mcp_registration import RESEARCH_MCP_TOOL_NAMES
    ...
    assert server.registered == [*RESEARCH_MCP_TOOL_NAMES, *EVALUATION_MCP_TOOL_NAMES]
...
@pytest.mark.unit
def test_evaluation_public_outcome_tool_writes_persisted_review(tmp_path: Path) -> None:
    store = _store(tmp_path)
    ...
    result = service.point_in_time_outcome_evaluation(
        run_id=RUN_ID,
        candidate_id=target.candidate_id,
...
@pytest.mark.unit
def test_evaluation_service_runs_live_tools_from_persisted_outcome_artifacts(
    tmp_path: Path,
) -> None:
    audit_dir = _persist_outcome_sources(tmp_path)
    service = EvaluationService(repo_root=tmp_path)

    loaded = service.load_outcome_evaluations(run_id=RUN_ID)
    ablation = service.signal_family_ablation(
        run_id=RUN_ID,
        cohort_id="evaluation-evalcal-stage7-msft-swing",
        point_in_time_cutoff=AS_OF.isoformat(),
        artifact_dir=audit_dir.as_posix(),
        families=("technicals",),
    )
    walk_forward = service.walk_forward_evaluation(
        run_id=RUN_ID,
        cohort_id="evaluation-evalcal-stage7-msft-swing",
        point_in_time_cutoff=AS_OF.isoformat(),
        artifact_dir=audit_dir.as_posix(),
    ...
    calibration = service.calibration_summary(
        run_id=RUN_ID,
        cohort_id="evaluation-evalcal-stage7-msft-swing",
    ...
    service.store.record_tool_run(
        ToolRunRecord(
            tool_run_id="tool-evaluation-report-integration-evidence",
            run_id=RUN_ID,
            tool_name="evaluation_report_integration_evidence",
    ...
    inspection = service.inspect_evaluation_run(run_id=RUN_ID)
    ...
    assert inspection["evaluation_tool_run_count"] == 7


@pytest.mark.unit
def test_evaluation_service_requires_persisted_outcome_artifacts(tmp_path: Path) -> None:
    _store(tmp_path)
    service = EvaluationService(repo_root=tmp_path)

    with pytest.raises(ValueError, match="No persisted outcome evaluations"):
        service.load_outcome_evaluations(run_id=RUN_ID)


@pytest.mark.unit
def test_evaluation_inferred_artifact_dir_still_uses_write_policy(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="Research artifact writes are limited"):
        service.calibration_summary(
            run_id=RUN_ID,
            cohort_id="evaluation-evalcal-disallowed-dir",
            as_of=AS_OF.isoformat(),
            bin_edges=(0.0, 0.5, 1.0),
        )
