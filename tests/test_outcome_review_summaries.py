...
def test_cross_run_prior_review_uses_persisted_outcome_payload(
    ...
    prior = service.run_offline_research_flow(
        run_date="2026-05-12",
        output_dir="reports/reliability-outcome-summaries",
        symbol="TSLA",
    )
    persisted = _write_persisted_prior_outcome(service, prior=prior)

    current = service.run_offline_research_flow(
        run_date="2026-05-13",
        output_dir="reports/reliability-outcome-summaries",
        symbol="TSLA",
    )
    ...
    freshness_records = review["metadata"]["reliability_freshness"]["evidence_aging_records"]
    assert freshness_records[0]["age_status"] == "stale"
...
def test_invalid_persisted_outcome_artifact_blocks_synthetic_prior_history(
    ...
    prior = service.run_offline_research_flow(
        run_date="2026-05-12",
        output_dir=f"reports/reliability-outcome-summaries-{failure_mode}",
        symbol="TSLA",
    )
    persisted = _write_persisted_prior_outcome(service, prior=prior)
...
def _write_persisted_prior_outcome(
    ...
    *,
    ...
    target = target.model_copy(
        update={
            "metadata": {**target.metadata, "reliability_freshness": reliability_freshness},
            "candidate_snapshot": {
    ...
    return write_point_in_time_outcome_evaluation_artifacts(
        ...
        artifact_dir=service.repo_root / "reports" / "reliability-outcome-summaries" / "audit",
        run_id=run_id,
        target=target,
...
def _record_outcome_evidence(service: ResearchService, *, run_id: str) -> None:
    service.store.record_tool_run(
        ToolRunRecord(
            tool_run_id="tool-tsla-prior-window-close",
            run_id=run_id,
            tool_name="reliability_prior_outcome_summary_test",
            tool_version="reliability.test.v1",
            status="successful",
            started_at=NOW,
            completed_at=NOW,
            inputs={"symbol": "TSLA"},
