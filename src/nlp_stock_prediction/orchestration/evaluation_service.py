...
from nlp_stock_prediction.evaluation.ablation import (
    EVALUATION_ABLATION_TOOL_NAME as ABLATION_ARTIFACT_TOOL_NAME,
)
from nlp_stock_prediction.evaluation.ablation import (
...
from nlp_stock_prediction.evaluation.calibration import (
    EVALUATION_CALIBRATION_TOOL_NAME as CALIBRATION_SUMMARY_ARTIFACT_TOOL_NAME,
)
from nlp_stock_prediction.evaluation.common import (
    digest,
    freshness_records_from_target,
    slug,
)
...
from nlp_stock_prediction.evaluation.walk_forward import (
    EVALUATION_WALK_FORWARD_TOOL_NAME as WALK_FORWARD_ARTIFACT_TOOL_NAME,
)
from nlp_stock_prediction.evaluation.walk_forward import (
...
from nlp_stock_prediction.orchestration.evaluation_sources import (
    OutcomeEvaluationSource,
    load_outcome_evaluation_sources,
)
from nlp_stock_prediction.orchestration.orchestration_common import ArtifactWritePolicy
...
EVALUATION_STAGE_ORDER: tuple[str, ...] = ("prepare", "evaluate", "summarize", "inspect")
EvaluationStage = Literal["prepare", "evaluate", "summarize", "inspect"]

EVALUATION_OUTCOME_EVALUATION_TOOL_ID = "evaluation.point_in_time_outcome_evaluation"
EVALUATION_LOAD_OUTCOME_EVALUATIONS_TOOL_ID = "evaluation.load_persisted_outcome_evaluations"
EVALUATION_ABLATION_TOOL_ID = "evaluation.signal_family_ablation"
EVALUATION_WALK_FORWARD_TOOL_ID = "evaluation.walk_forward_evaluation"
EVALUATION_CALIBRATION_TOOL_ID = "evaluation.calibration_summary"
EVALUATION_INSPECT_TOOL_ID = "evaluation.inspect_run"
RELIABILITY_LIVE_OUTCOME_MATERIALIZATION_TOOL_ID = "reliability.live_outcome_materialization"
EVALUATION_OUTCOME_EVALUATION_PUBLIC_TOOL_NAME = "point_in_time_outcome_evaluation"
RELIABILITY_LIVE_OUTCOME_MATERIALIZATION_PUBLIC_TOOL_NAME = "live_outcome_materialization"
EVALUATION_LOAD_OUTCOME_EVALUATIONS_TOOL_NAME = "load_outcome_evaluations"
EVALUATION_INSPECT_TOOL_NAME = "inspect_evaluation_run"
...
class EvaluationToolMetadata(ContractModel):
    """Canonical metadata for one live Evaluation Stage evaluation tool."""
    ...
    def validate_tool_metadata(self) -> EvaluationToolMetadata:
        if self.tool_id in self.dependencies:
            raise ValueError("evaluation tools cannot depend on themselves")
        if len(set(self.dependencies)) != len(self.dependencies):
            raise ValueError("evaluation tool dependencies must be unique")
        return self
...
class EvaluationToolRegistry:
    """Deterministic Evaluation Stage tool metadata registry."""

    def __init__(self, tools: Iterable[EvaluationToolMetadata] = ()) -> None:
        self._tools: dict[str, EvaluationToolMetadata] = {}
        for tool in tools:
            self.register(tool)

    def register(self, tool: EvaluationToolMetadata) -> None:
        if tool.tool_id in self._tools:
            raise ValueError(f"evaluation tool is already registered: {tool.tool_id}")
        if any(
            item.tool_name == tool.tool_name and item.tool_version == tool.tool_version
            for item in self._tools.values()
        ):
            raise ValueError(
                f"evaluation tool name/version is already registered: "
                f"{tool.tool_name}@{tool.tool_version}"
            )
        self._tools[tool.tool_id] = tool

    def get(self, tool_id: str) -> EvaluationToolMetadata:
        try:
            return self._tools[tool_id]
        except KeyError as exc:
            raise KeyError(f"evaluation tool is not registered: {tool_id}") from exc

    def specs(self) -> tuple[EvaluationToolMetadata, ...]:
        return self.ordered_for_stages(EVALUATION_STAGE_ORDER)

    def ordered_for_stages(
        self,
        stage_order: Sequence[str] = EVALUATION_STAGE_ORDER,
    ) -> tuple[EvaluationToolMetadata, ...]:
        ...
        if unknown_stages:
            ...
            raise ValueError(f"evaluation tools use stages missing from stage_order: {joined}")
    ...
    def as_plan(self) -> JsonObject:
        return {
            ...
            "stage_order": list(EVALUATION_STAGE_ORDER),
        }
    ...
    def _validate_dependencies(self) -> None:
        ...
        if missing:
            ...
            raise ValueError(f"evaluation tool dependencies are not registered: {details}")
    ...
    return EvaluationToolRegistry(
        (
            EvaluationToolMetadata(
                ...
                metadata={"source_tool_name": RELIABILITY_LIVE_OUTCOME_TOOL_NAME},
            ),
            EvaluationToolMetadata(
                ...
                metadata={"source_tool_name": EVALUATION_OUTCOME_TOOL_NAME},
            ),
            EvaluationToolMetadata(
            ...
            EvaluationToolMetadata(
                ...
                tool_version=EVALUATION_ABLATION_TOOL_VERSION,
                stage="evaluate",
                description=(
                ...
                dependencies=(EVALUATION_LOAD_OUTCOMES_TOOL_ID,),
                metadata={"source_tool_name": ABLATION_ARTIFACT_TOOL_NAME},
            ),
            EvaluationToolMetadata(
                ...
                dependencies=(EVALUATION_LOAD_OUTCOMES_TOOL_ID,),
                metadata={"source_tool_name": WALK_FORWARD_ARTIFACT_TOOL_NAME},
            ),
            EvaluationToolMetadata(
                ...
                dependencies=(EVALUATION_LOAD_OUTCOMES_TOOL_ID,),
                metadata={"source_tool_name": CALIBRATION_SUMMARY_ARTIFACT_TOOL_NAME},
            ),
            EvaluationToolMetadata(
                ...
                metadata={"source_tool_name": RELIABILITY_CALIBRATION_DRIFT_TOOL_NAME},
            ),
            EvaluationToolMetadata(
...
def evaluation_tool_plan(
    registry: EvaluationToolRegistry | None = None,
) -> JsonObject:
    """Return a registry-derived public evaluation tool plan."""

    return (registry or build_evaluation_tool_registry()).as_plan()


@dataclass(frozen=True)
class EvaluationService:
    """Stateful Evaluation Stage service over the research SQLite run graph."""

    repo_root: Path = Path(".")
    database_path: Path = DEFAULT_RESEARCH_DATABASE_PATH
    extra_write_roots: tuple[Path, ...] = ()
    registry: EvaluationToolRegistry = field(default_factory=build_evaluation_tool_registry)
    ...
    def list_evaluation_tool_plan(self) -> JsonObject:
        return evaluation_tool_plan(self.registry)
    ...
    def evaluation_materialize_outcome(
        ...
        *,
        ...
        return self.live_outcome_materialization(
            run_id=run_id,
    ...
    def evaluation_load_outcomes(self, *, run_id: str) -> JsonObject:
        return self.load_outcome_evaluations(run_id=run_id)
    ...
    def evaluation_ablation(
        ...
        *,
        ...
        required_artifact_dir = _required_artifact_dir(artifact_dir)
        return self.signal_family_ablation(
            run_id=run_id,
            cohort_id=cohort_id,
            point_in_time_cutoff=point_in_time_cutoff,
            artifact_dir=required_artifact_dir,
    ...
    def evaluation_walk_forward(
        ...
        *,
        ...
        required_artifact_dir = _required_artifact_dir(artifact_dir)
        return self.walk_forward_evaluation(
            run_id=run_id,
            cohort_id=cohort_id,
            point_in_time_cutoff=point_in_time_cutoff,
    ...
    def evaluation_calibration_drift(
        ...
        *,
        ...
        required_artifact_dir = _required_artifact_dir(artifact_dir)
        return self.calibration_drift_check(
            run_id=run_id,
    ...
    def evaluation_inspect(self, *, run_id: str) -> JsonObject:
        return self.inspect_evaluation_run(run_id=run_id)

    def live_outcome_materialization(
        self,
        *,
        run_id: str,
        candidate_id: str,
    ...
    def point_in_time_outcome_evaluation(
        self,
        *,
        run_id: str,
        candidate_id: str,
    ...
    def load_outcome_evaluations(self, *, run_id: str) -> JsonObject:
        self._require_run(run_id)
    ...
    def inspect_evaluation_run(self, *, run_id: str) -> JsonObject:
    ...
    def _outcome_sources(self, run_id: str) -> tuple[OutcomeEvaluationSource, ...]:
        return load_outcome_evaluation_sources(
            store=self.store,
            repo_root=self.repo_root,
            run_id=run_id,
        )
...
def _registry_tool_run_names(registry: EvaluationToolRegistry) -> set[str]:
    names: set[str] = set()
...
def _outcome_review_summary_from_source(
    ...
    *,
    ...
    reliability_records = freshness_records_from_target(target)
    ...
    limitations = tuple(
        dict.fromkeys(
            (
                ...
                *reliability_records.limitations,
            )
        )
    )
    ...
    return OutcomeReviewSummary(
        ...
        evidence_aging_records=reliability_records.evidence_aging_records,
        artifact_freshness_reviews=reliability_records.artifact_freshness_reviews,
...
__all__ = [
    ...
    "EVALUATION_LOAD_OUTCOME_EVALUATIONS_TOOL_NAME",
    "EVALUATION_MATERIALIZE_OUTCOME_TOOL_NAME",
    "EVALUATION_OUTCOME_EVALUATION_PUBLIC_TOOL_NAME",
    ...
    "RELIABILITY_LIVE_OUTCOME_MATERIALIZATION_PUBLIC_TOOL_NAME",
    "RELIABILITY_LIVE_OUTCOME_MATERIALIZATION_TOOL_ID",
    "EvaluationService",
    "EvaluationToolMetadata",
