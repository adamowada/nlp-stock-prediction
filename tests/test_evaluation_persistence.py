...
def _seed_prediction_and_evaluation_runs(store: SQLiteStore) -> None:
    ...
    store.record_tool_run(
        ToolRunRecord(
            ...
            run_id="run-msft-evaluation-2026-05-14",
            tool_name="reliability_outcome_review",
            tool_version="0.1",
            status="ok",
            started_at=_ts(11),
            completed_at=_ts(12),
    ...
    store.record_artifact(
        ArtifactRecord(
            ...
            produced_by="reliability_outcome_review",
            record_count=1,
            metadata={"report_data_mode": "live", "provider_mode": "live"},
            created_at=_ts(12),
        )
...
@pytest.mark.unit
def test_reliability_schema_initializes_persistence_tables_idempotently(tmp_path: Path) -> None:
...
@pytest.mark.unit
def test_reliability_attempts_preserve_repeated_outcome_reviews_without_overwrite(
    tmp_path: Path,
) -> None:
    store = _store(tmp_path)
    store.initialize()
...
@pytest.mark.unit
def test_reliability_calibration_source_links_and_drift_records_are_queryable(
    tmp_path: Path,
) -> None:
    store = _store(tmp_path)
    store.initialize()
    ...
    calibration = CalibrationRunRecord(
        ...
        run_id="run-msft-evaluation-2026-05-14",
        tool_run_id="tool-msft-outcome-2026-05-14",
        method_version="reliability-evaluation-hardening.v1",
...
@pytest.mark.unit
def test_reliability_prediction_outcomes_reject_impossible_result_shapes(tmp_path: Path) -> None:
    store = _store(tmp_path)
    store.initialize()
    _seed_prediction_and_evaluation_runs(store)
...
@pytest.mark.unit
def test_reliability_live_persistence_rejects_non_live_modes(tmp_path: Path) -> None:
    store = _store(tmp_path)
    store.initialize()
    _seed_prediction_and_evaluation_runs(store)
...
@pytest.mark.unit
def test_reliability_failed_tool_cleanup_keeps_prior_successful_attempts(
    tmp_path: Path,
) -> None:
    store = _store(tmp_path)
    store.initialize()
