"""Research Stage sector and macro context tool."""
...
RESEARCH_SECTOR_MACRO_SCHEMA_VERSION = "research.sector-macro.v1"
RESEARCH_SECTOR_MACRO_TOOL_NAME = "research_sector_macro"
RESEARCH_SECTOR_MACRO_TOOL_SLUG = "research-sector-macro"
_SECTOR_INPUT_PROVIDER = "research-sector-inputs"


@dataclass(frozen=True)
class ResearchSectorMacroTool:
    ...
    def run(
        ...
        *,
        ...
        tool_run_id, artifact_id, filename = tool_identity(
            tool_slug=RESEARCH_SECTOR_MACRO_TOOL_SLUG,
            run_id=run_id,
            symbol=normalized_symbol,
        )
        macro_request = MacroRequest(
            request_id=f"research-macro-{run_id}-{normalized_symbol}",
            run_date=run_date,
        ...
        with safe_research_tool_execution(
            ...
            artifact_path = write_research_json_artifact(
                ...
                produced_by=RESEARCH_SECTOR_MACRO_TOOL_NAME,
                tool_run_id=tool_run_id,
                schema_version=RESEARCH_SECTOR_MACRO_SCHEMA_VERSION,
                artifact_id=artifact_id,
...
def run_research_sector_macro_tool(
    *,
    store: SQLiteStore,
    repo_root: Path,
    artifact_dir: Path,
    ...
    return ResearchSectorMacroTool(macro_providers=tuple(macro_providers)).run(
        store=store,
        repo_root=repo_root,
        artifact_dir=artifact_dir,
        run_id=run_id,
...
def _record_sector_source_query(
    *,
    ...
    source_query_id = (
        f"query-{RESEARCH_SECTOR_MACRO_TOOL_SLUG}-"
    ...
    record = SourceQueryRecord(
        ...
        metadata=cast(
            JsonObject,
            {
                "research_tool": RESEARCH_SECTOR_MACRO_TOOL_SLUG,
                "sector": sector,
                "benchmark_symbol": benchmark_symbol,
            },
        ),
...
__all__ = [
    "RESEARCH_SECTOR_MACRO_SCHEMA_VERSION",
