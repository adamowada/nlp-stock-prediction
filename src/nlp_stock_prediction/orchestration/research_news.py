"""Research Stage news and catalyst evidence tool."""
...
RESEARCH_NEWS_SCHEMA_VERSION = "research.news-catalyst.v1"
RESEARCH_NEWS_TOOL_NAME = "research_news_catalyst"
RESEARCH_NEWS_TOOL_SLUG = "research-news-catalyst"


@dataclass(frozen=True)
class ResearchNewsCatalystTool:
    ...
    def run(
        ...
        *,
        ...
        request = EvidenceRequest(
            request_id=f"research-news-{run_id}-{normalized_symbol}",
            run_date=run_date,
            tickers=tickers,
            limit=limit,
        ...
        tool_run_id, artifact_id, filename = tool_identity(
            tool_slug=RESEARCH_NEWS_TOOL_SLUG,
            run_id=run_id,
        ...
        with safe_research_tool_execution(
            ...
            provider_results = tuple(
                provider.fetch_articles(request) for provider in self.providers
            )
            ...
            artifact_path = write_research_json_artifact(
                ...
                produced_by=RESEARCH_NEWS_TOOL_NAME,
                tool_run_id=tool_run_id,
                schema_version=RESEARCH_NEWS_SCHEMA_VERSION,
                artifact_id=artifact_id,
...
def run_research_news_catalyst_tool(
    *,
    store: SQLiteStore,
    repo_root: Path,
    artifact_dir: Path,
    ...
    return ResearchNewsCatalystTool(providers=tuple(providers)).run(
        store=store,
        repo_root=repo_root,
        artifact_dir=artifact_dir,
        run_id=run_id,
