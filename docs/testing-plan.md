...
Instrument-Universe Stage SQLite gates must cover:
...
Universe-discovery fixtures should be small and contract-shaped. Current Instrument-Universe Stage fixtures live under
...
### Instrument-Universe Stage Instrument Universe Gates

Focused Instrument-Universe Stage checks should run when instrument contracts, report contracts, storage schema, or
fixture shapes change:

```sh
python -m pytest tests/test_instrument_contracts.py
python -m pytest tests/test_storage_sqlite.py -k "instrument or watchlist or tradability"
python -m pytest tests/test_schema_contracts.py -k "resolution or report"
```
...
### Research Stage QA Gates

The Research Stage gate is deterministic and fixture-backed, and it exercises the real public tool
modules and Research Stage service path. The production-gate E2E
starts a Research Stage run, executes universe discovery, market data, technical package, social evidence,
...
python -m pytest tests/test_research_tool_suite_e2e.py tests/test_research_public_wiring.py tests/test_research_service.py tests/test_research_batch.py tests/test_wsb_trending_batch.py
```
...
ranking artifacts, WSB trending-discovery artifacts, audit manifest coverage, prediction-quality
evaluation artifacts, Research Stage artifact type alignment, and visible recovery from partial tool
failure.
...
```sh
python -m pytest tests/test_report_assembly.py tests/test_report_data_modes.py
python -m pytest tests/test_prior_outcomes.py tests/test_json_report_index.py
python -m pytest tests/test_reporting_markdown.py tests/test_schema_contracts.py -k "report or provenance"
```
...
### Evaluation Stage QA Gates

The Evaluation Stage gate is deterministic and uses persisted point-in-time outcome evaluations rather than
...
```sh
python -m pytest tests/test_evaluation_contracts.py tests/test_outcome_evaluation_tool.py
python -m pytest tests/test_evaluation_public_tooling.py tests/test_evaluation_report_integration.py
python -m pytest tests/test_calibration_summary.py tests/test_signal_family_ablation.py tests/test_walk_forward_evaluation.py
```
...
attribution, artifact write-policy enforcement, and report references that preserve Evaluation Stage outputs
without turning calibration into trading-performance claims.

### Reliability Stage Evaluation Hardening Gates

The Reliability Stage gate is deterministic unless an individual live test is explicitly opted in. It exercises
...
```sh
python -m pytest tests/test_artifact_freshness.py tests/test_evidence_aging.py
python -m pytest tests/test_live_outcome_materialization.py tests/test_outcome_review_summaries.py
python -m pytest tests/test_source_reliability.py tests/test_provider_playbooks.py tests/test_provider_reliability.py
python -m pytest tests/test_calibration_drift.py tests/test_calibration_summary.py tests/test_walk_forward_evaluation.py
python -m pytest tests/test_reliability_public_tooling.py tests/test_evaluation_public_tooling.py tests/test_research_public_wiring.py
python -m pytest tests/test_live_validation.py tests/test_evaluation_persistence.py tests/test_storage_sqlite.py
```
...
The public-tooling gate must verify the public `evaluation` CLI, explicit existing database
...
```sh
python -m pytest tests/test_documentation_contracts.py tests/test_schema_contracts.py
python -m pytest tests/test_research_providers.py tests/test_apnews_provider.py tests/test_candlecharts_provider.py
python -m pytest tests/test_codex_smoke_mcp_service.py tests/test_orchestration_runtime.py
...
The Research Stage Codex smoke is opt-in because it launches the real Codex CLI and may use live web
search. The script is still named `scripts/run_codex_smoke.py` for compatibility, but it
drives the Research Stage MCP tool suite. Mark tests with `codex_smoke` and skip unless
