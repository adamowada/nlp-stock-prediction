"""Prediction candidate synthesis for Codex Smoke Stage smoke runs."""
...
def synthesize_prediction_candidates(
    *,
    ...
    candidate = PredictionCandidateRecord(
        ...
        metadata={"codex_smoke_mcp": True, "symbol": symbol.upper()},
    )
    payload: JsonObject = {
        "schema_version": "codex_smoke-candidate-synthesis.v1",
        "run_id": run_id,
        "records": [
            {
    ...
    for evidence_id in candidate.evidence_for:
        store.link_candidate_evidence(
            CandidateEvidenceLinkRecord(
                ...
                metadata={"source": "codex_smoke_mcp_synthesis"},
                created_at=now,
            )
        )
...
def _candidate_scenario(*, symbol: str, evidence_for: bool, evidence_against: bool) -> str:
    ...
    if evidence_for and evidence_against:
        return (
            f"Live-search evidence for {normalized} is contradictory, so the Codex Smoke Stage "
