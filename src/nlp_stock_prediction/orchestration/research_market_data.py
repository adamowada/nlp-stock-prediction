"""Research Stage market-data tool backed by provider-result contracts."""
...
RESEARCH_MARKET_DATA_SCHEMA_VERSION = "research.market-data.v1"
RESEARCH_MARKET_DATA_TOOL_NAME = "research_market_data"
RESEARCH_MARKET_DATA_TOOL_VERSION = RESEARCH_MARKET_DATA_SCHEMA_VERSION
...
class ResearchMarketDataArtifact(ContractModel):
    """Stable JSON payload written by the Research Stage market-data tool."""

    schema_version: NonEmptyStr = RESEARCH_MARKET_DATA_SCHEMA_VERSION
    run_id: NonEmptyStr
...
class ResearchMarketDataProviderTool(Protocol):
    ...
    def run(
        ...
        *,
        ...
        with safe_research_tool_execution(
            ...
            tool_version=RESEARCH_MARKET_DATA_TOOL_VERSION,
            ...
            artifact = ArtifactIndex.for_directory(
                ...
                produced_by=RESEARCH_MARKET_DATA_TOOL_NAME,
                tool_run_id=resolved_tool_run_id,
                schema_version=RESEARCH_MARKET_DATA_SCHEMA_VERSION,
...
def build_market_data_artifact(
    *,
    ...
    return ResearchMarketDataArtifact(
        run_id=run_id,
...
def market_data_artifact_payload(artifact: ResearchMarketDataArtifact) -> JsonObject:
    """Serialize a Research Stage market-data artifact to stable JSON-compatible data."""
...
def load_research_market_data_artifact(path: Path) -> ResearchMarketDataArtifact:
    """Load a previously written market-data artifact."""

    return ResearchMarketDataArtifact.model_validate_json(path.read_text(encoding="utf-8"))
...
def _market_data_request_id(run_id: str, symbol: str) -> str:
    return f"research-market-data-{stable_digest(f'{run_id}:{symbol}')}"


def _market_data_tool_run_id(run_id: str, symbol: str) -> str:
    return f"tool-research-market-data-{stable_digest(f'{run_id}:{symbol}')}"
...
__all__ = [
    "RESEARCH_MARKET_DATA_SCHEMA_VERSION",
    "RESEARCH_MARKET_DATA_TOOL_NAME",
    "RESEARCH_MARKET_DATA_TOOL_VERSION",
    "MarketDataProvenance",
    "MarketDataToolResult",
    "ResearchMarketDataArtifact",
    "ResearchMarketDataProviderTool",
