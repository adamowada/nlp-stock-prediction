...
RUN_ID = "research-market-data-run"
...
def _store(tmp_path: Path, run_id: str = RUN_ID) -> SQLiteStore:
    ...
    store.upsert_research_run(
        ResearchRunRecord(
            ...
            objective="exercise Research Stage market data tool",
            status="running",
            started_at=NOW,
            metadata={
...
@pytest.mark.unit
def test_research_market_data_tool_writes_artifact_and_sqlite_rows(tmp_path: Path) -> None:
    ...
    assert result.artifact_payload.schema_version == RESEARCH_MARKET_DATA_SCHEMA_VERSION
    ...
    loaded = load_research_market_data_artifact(Path(result.artifact.path))
    ...
    assert artifact.schema_version == RESEARCH_MARKET_DATA_SCHEMA_VERSION
    assert artifact.produced_by == "research_market_data"
...
@pytest.mark.unit
def test_research_market_data_rolls_back_file_and_rows_when_artifact_index_fails(
    tmp_path: Path,
) -> None:
    ...
    store.upsert_research_run(
        ResearchRunRecord(
            run_id=RUN_ID,
            run_kind="research_tool_test",
            objective="exercise Research Stage market data rollback",
            status="running",
            started_at=NOW,
            metadata={
                "run_date": RUN_DATE.isoformat(),
...
def test_research_market_data_tool_records_warning_artifacts_for_provider_problems(
    tmp_path: Path,
...
@pytest.mark.unit
def test_research_market_data_rejects_snapshot_ticker_mismatch(tmp_path: Path) -> None:
    store = _store(tmp_path)
...
@pytest.mark.unit
def test_research_market_data_filters_mismatched_and_future_bars(tmp_path: Path) -> None:
    store = _store(tmp_path)
...
@pytest.mark.unit
def test_research_market_data_non_fixture_provider_does_not_default_to_fixture_provenance(
    tmp_path: Path,
) -> None:
    store = _store(tmp_path)
...
@pytest.mark.unit
def test_research_market_data_uses_provider_specific_source_url_for_fallback_provider(
    tmp_path: Path,
) -> None:
    store = _store(tmp_path)
