"""MCP-facing Codex Smoke Stage module for real Codex smoke runs."""
...
from nlp_stock_prediction.orchestration.codex_smoke_dummy_tools import (
    run_codex_smoke_dummy_analysis_tool,
    run_codex_smoke_dummy_universe_tool,
)
from nlp_stock_prediction.orchestration.codex_smoke_evidence import record_codex_search_evidence
from nlp_stock_prediction.orchestration.codex_smoke_synthesis import (
    synthesize_prediction_candidates,
)
from nlp_stock_prediction.orchestration.codex_smoke_tool_plan import codex_smoke_research_tool_plan
...
from nlp_stock_prediction.orchestration.report_builder import render_prediction_report
...
class CodexSmokeMcpService:
    """Stateful Codex Smoke Stage interface exposed through MCP and tests."""
    ...
    def write_policy(self) -> ArtifactWritePolicy:
        return ArtifactWritePolicy(self.repo_root)
    ...
    def start_research_run(
        ...
        *,
        ...
        run_id = codex_smoke_run_id(parsed_date, normalized_symbol)
        ...
        self.store.upsert_research_run(
            ResearchRunRecord(
                ...
                metadata={
                    ...
                    "stage": "codex_smoke_codex_orchestrator",
    ...
    def list_research_tool_plan(self) -> JsonObject:
        return codex_smoke_research_tool_plan()
    ...
    def run_dummy_universe_tool(self, *, run_id: str, symbol: str) -> JsonObject:
        def action(
            ...
            return run_codex_smoke_dummy_universe_tool(
                store=self.store,
                repo_root=self.repo_root,
                paths=paths,
                run_id=run_id,
    ...
    def run_dummy_analysis_tool(self, *, run_id: str, symbol: str) -> JsonObject:
        def action(
            ...
            return run_codex_smoke_dummy_analysis_tool(
                store=self.store,
                repo_root=self.repo_root,
                paths=paths,
                run_id=run_id,
    ...
    def render_prediction_report(self, *, run_id: str, symbol: str) -> JsonObject:
        def action(
            ...
            return render_prediction_report(
                store=self.store,
                repo_root=self.repo_root,
                run=run,
                paths=paths,
    ...
    def _run_symbol_step(
        ...
        *,
        run_id: str,
        symbol: str,
        action: Callable[[ResearchRunRecord, str, ReportRunPaths, date], JsonObject],
    ) -> JsonObject:
        run = self._require_run(run_id)
    ...
    def _paths(self, run_date: date, output_dir: str) -> ReportRunPaths:
...
__all__ = ["ALLOWED_WRITE_ROOTS", "CodexSmokeMcpService", "ReportRunPaths"]
