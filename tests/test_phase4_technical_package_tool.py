...
RUN_ID = "phase4-technical-package-run"
...
@pytest.mark.unit
def test_phase4_technical_package_computes_indicators_and_indexes_artifact(
    tmp_path: Path,
) -> None:
    ...
    assert artifact_payload.schema_version == PHASE4_TECHNICAL_PACKAGE_SCHEMA_VERSION
    ...
    loaded = load_phase4_technical_package_artifact(Path(result.artifact.path))
    ...
    assert artifact.schema_version == PHASE4_TECHNICAL_PACKAGE_SCHEMA_VERSION
    assert artifact.metadata["timesfm_applied_to_analysis"] is True
    assert artifact in store.list_artifacts_for_run(RUN_ID)


@pytest.mark.unit
def test_phase4_technical_package_does_not_apply_timesfm_without_market_bars(
    tmp_path: Path,
) -> None:
    store = _store(tmp_path)
...
@pytest.mark.unit
def test_phase4_technical_package_malformed_market_artifact_path_warns_not_crashes(
    tmp_path: Path,
) -> None:
    store = _store(tmp_path)
