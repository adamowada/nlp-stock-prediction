...
from nlp_stock_prediction.orchestration import OFFLINE_FIXTURE_REPORT_DATA_MODE, ResearchService
...
def test_first_indexed_report_records_unavailable_prior_outcome(tmp_path: Path) -> None:
    ...
    result = service.run_offline_research_flow(
        run_date="2026-05-13",
        output_dir="reports/report-prior-first-run",
        symbol="TSLA",
    )
...
def test_supported_follow_up_evidence_reviews_prior_report_artifact(tmp_path: Path) -> None:
    ...
    prior = service.run_offline_research_flow(
        run_date="2026-05-12",
        output_dir="reports/report-prior-supported",
        symbol="TSLA",
    )
...
def test_stale_prior_report_becomes_explicit_limitation(tmp_path: Path) -> None:
    ...
    prior = service.run_offline_research_flow(
        run_date="2026-05-12",
        output_dir="reports/report-prior-stale",
        symbol="TSLA",
    )
...
def test_contradictory_follow_up_evidence_reviews_prior_report(tmp_path: Path) -> None:
    ...
    prior = service.run_offline_research_flow(
        run_date="2026-05-12",
        output_dir="reports/report-prior-contradicted",
        symbol="TSLA",
    )
...
def _insert_contradictory_candidate(service: ResearchService, *, run_id: str) -> None:
    ...
    service.store.record_tool_run(
        ToolRunRecord(
            ...
            run_id=run_id,
            tool_name="report_prior_outcome_test_evidence",
            tool_version="report.prior-outcome.v1",
            status="successful",
            started_at=NOW,
            completed_at=NOW,
            inputs={
