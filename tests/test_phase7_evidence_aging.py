    ...
    for artifact_id, created_at, metadata in artifact_inputs:
        store.record_artifact(
            ArtifactRecord(
                ...
                sha256="d" * 64,
                schema_version="phase7-test.v1",
                produced_by="phase7_aging_review",
                record_count=1,
...
def test_target_freezing_carries_structured_aging_and_freshness_metadata(
    ...
    freshness = target.metadata["phase7_freshness"]
...
@pytest.mark.unit
def test_cohort_selection_carries_structured_phase7_records(tmp_path: Path) -> None:
    store = _store(tmp_path)
    target = build_prediction_evaluation_target(
        store=store,
        run_id=RUN_ID,
    ...
    assert {record.evidence_id for record in cohort.phase7_evidence_aging_records} == {
        "evidence-msft-old-news",
        "evidence-msft-provider-replaced",
    }
